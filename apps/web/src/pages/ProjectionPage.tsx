        ...
        <>
          ...
          {safe && (
            <div className="rounded-2xl border border-emerald-800/40 bg-emerald-500/5 p-5">
              <p className="text-xs uppercase tracking-wide text-emerald-400/80 mb-1">Limite saudável de gasto este mês</p>
              <strong className={`text-3xl ${safe.amount >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{fmt(safe.amount)}</strong>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <ComposedChart data={chartData}>
                  ...
                  <Bar dataKey="Entradas" fill="#22c55e" radius={[4, 4, 0, 0]} barSize={14} />
                  <Bar dataKey="Saídas" fill="#ef4444" radius={[4, 4, 0, 0]} barSize={14} />
                  <Area type="monotone" dataKey="Saldo" stroke="#34d399" strokeWidth={2.5} fill="url(#saldoFill)" />
          ...
          {projection.length > 0 && (
            <div className="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-800"><h2 className="font-semibold">Mês a mês</h2></div>
              {projection.map((m: any) => (
                <div key={m.key} className="flex items-center justify-between px-5 py-3 border-b border-slate-800 last:border-0">
                  <span className="text-sm font-medium capitalize">{m.label}</span>
                  <div className="flex items-center gap-5">
                    <span className="flex items-center gap-1 text-sm text-emerald-400"><TrendingUp className="h-3.5 w-3.5" />{fmt(m.income)}</span>
                    <span className="flex items-center gap-1 text-sm text-rose-400"><TrendingDown className="h-3.5 w-3.5" />{fmt(m.expense)}</span>
                    <span className="text-xs text-slate-500 w-28 text-right">{Math.round(m.committedRatio * 100)}% comprometido</span>
                    <span className={`text-sm font-semibold w-28 text-right ${m.closingBalance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <input value={description} onChange={e => setDescription(e.target.value)} placeholder="O quê? (ex: Notebook)"
          className="sm:col-span-2 rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100" />
      ...
      <button
        ...
        className="mt-3 rounded-lg bg-sky-500/90 px-4 py-2 text-sm font-medium text-white hover:bg-sky-500 disabled:opacity-40"
      >
